...
func (r *TicketRepository) GetTicketByID(ticketID uuid.UUID) (*models.Ticket, error) {
	...
	err := r.db.Preload("Node").Preload("ResourcePool").Preload("Resources.Resource.ResourceType").Preload("ResourcePool.Organization").Preload("Namespace.Project").First(&ticket, "id = ?", ticketID).Error
	if err != nil {
		return nil, err
	}
	return &ticket, nil
...
func (r *TicketRepository) GetTicketsByUserID(userID uuid.UUID) ([]models.Ticket, error) {
	var tickets []models.Ticket
	err := r.db.Preload("Resources").Preload("Namespace.Project").Where("owner_id = ?", userID).Order("created_at DESC").Find(&tickets).Error
	if err != nil {
		return nil, err
	}
	return tickets, nil
...
func (r *TicketRepository) StopPendingTicket(ticketID uuid.UUID) error {
	return r.db.Model(&models.Ticket{}).Where("id = ?", ticketID).Updates(map[string]interface{}{
		"status": "created", "started_time": nil,
	}).Error
}
